...
export default function ContactPage(){return <><Header/><main className="site-page contact-page">
  ...
  <div className="contact-columns">
    <section className="contact-info panel"><div className="contact-details">{[["ساعات کاری","شنبه تا پنجشنبه، ۸ تا ۱۸","clock"],["آدرس شرکت","تهران، بلوار میرداماد، خیابان کازرون شمالی، خیابان نیک‌رای، پلاک ۲، طبقه سوم، واحد ۶","pin"],["ایمیل","info@kaghaz20.ir","mail"],["تماس تلفنی",SUPPORT_PHONE.display,"phone",SUPPORT_PHONE.tel]].map(x=>{const content=<><PageIcon name={x[2]}/><span><b>{x[0]}</b><small>{x[1]}</small></span></>; return x[3]?<a key={x[0]} href={x[3]}>{content}</a>:<div key={x[0]}>{content}</div>;})}</div><div className="map"><iframe src={mapEmbed} title="موقعیت شرکت کاغذ ۲۰ روی نقشه" loading="lazy"/></div><h3>شرکت کاغذ ۲۰</h3><p>تهران، بلوار میرداماد، خیابان کازرون شمالی، خیابان نیک‌رای، پلاک ۲، طبقه سوم، واحد ۶</p><div className="map-links">{mapLinks.map(([label,href])=><a key={label} href={href} target="_blank" rel="noopener noreferrer">مسیریابی با {label}</a>)}</div></section>
