...
export function ProductCard({ product, onAdd }: { product: Product; onAdd:(product:Product)=>void }) {
  ...
  return <article className="relative flex min-h-[280px] min-w-0 flex-col overflow-hidden rounded-xl border border-borderBlue bg-white p-2.5 shadow-soft transition sm:min-h-[300px] sm:p-3 sm:hover:-translate-y-1 sm:hover:shadow-card">
    ...
    <div className="relative z-10 mt-auto grid grid-cols-[1fr_52px] gap-3"><button type="button" onClick={(event)=>{event.preventDefault();event.stopPropagation();onAdd(product)}} className="grid h-11 touch-manipulation place-items-center rounded-lg bg-navy text-white" aria-label="افزودن به سبد"><CartIcon className="pointer-events-none size-5" /></button><Link className="grid h-11 place-items-center rounded-lg border border-borderBlue text-lg" href={`/products/${product.slug}`} aria-label="مشاهده سریع">◉</Link></div>
  </article>;
}
