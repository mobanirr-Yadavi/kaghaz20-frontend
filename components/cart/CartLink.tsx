...
export function CartLink({ mobile = false }: { mobile?: boolean }) {
  ...
  return <Link className={mobile ? "relative flex min-w-0 flex-col items-center justify-center gap-1 rounded-xl py-1.5 text-[10px] font-black text-navy" : "relative grid size-11 place-items-center rounded-full border border-transparent transition duration-200 hover:border-borderBlue hover:bg-softBlue hover:shadow-soft active:scale-95"} href="/cart" aria-label={`سبد خرید، ${shown} کالا`}><CartIcon className={mobile ? "size-5" : "size-6"} />{mobile && "سبد خرید"}{shown > 0 && <span className="absolute -right-1 -top-1 grid min-w-5 place-items-center rounded-full bg-buttonGold px-1 text-[10px] leading-5 text-navy">{new Intl.NumberFormat("fa-IR").format(shown)}</span>}</Link>;
}
