...
export function CartPageClient() {
  ...
  useEffect(() => {
    ...
    const restored = products.filter((product) => saved[product.id]).map((product) => ({ id: product.id, productId: product.id, title: product.title, subtitle: `${product.size} / ${product.weight} / ${product.sheets}`, image: product.image, price: product.priceValue, quantity: saved[product.id] }));
    if (restored.length) setItems(restored);
  }, []);

  useEffect(() => {
    if (!items.length) localStorage.removeItem("kaghaz20-cart");
    else localStorage.setItem("kaghaz20-cart", JSON.stringify(Object.fromEntries(items.map((item) => [item.productId, item.quantity]))));
  }, [items]);
    ...
    <div className="target-columns grid gap-5 lg:grid-cols-[320px_1fr]">
      ...
      <div className="order-1 space-y-5 lg:order-2">
        {items.length ? <CartTable
          items={items}
          updateQuantity={(id, quantity) => setItems((prev) => prev.map((item) => item.id === id ? { ...item, quantity } : item))}
