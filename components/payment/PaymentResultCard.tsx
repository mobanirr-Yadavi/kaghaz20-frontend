...
export function PaymentResultCard({ success, trackingCode, code }: { success: boolean; trackingCode?: string; code?: string }) {
  ...
  return (
    <section className="mx-auto max-w-xl rounded-2xl bg-white p-6 text-center shadow-card sm:p-10">
      <div className={`mx-auto grid size-20 place-items-center rounded-full text-4xl ${success ? "bg-emerald-50 text-emerald-600" : "bg-red-50 text-red-600"}`}>{success ? "✓" : "×"}</div>
      <h1 className="mt-5 text-2xl font-black text-navy">{success ? "پرداخت با موفقیت انجام شد" : "پرداخت ناموفق بود"}</h1>
